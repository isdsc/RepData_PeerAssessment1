...
To show the daily activity pattern, we will plot the average daily steps
...
# Index of the interval with max average steps
...
On the average, the maximum number of steps were taken at **`r max_steps_interval`**
with **`r prettyNum(max_steps, big.mark = ",")`** steps:

```{r}
# Let's show the max steps on the plot
activity +
  geom_segment(
    linetype = "longdash",
    colour = "blue",
    aes(
      x = max_step_index, y = -Inf,
      xend = max_step_index, yend = max_steps)
  ) +
  geom_text(
